	...
	if ( class_exists( 'PrdctfltrInit' ) ) {
        ...
        <nav class="woocommerce-pagination prdctfltr-pagination <?php echo esc_attr( $loadmore_nav_class ); ?>">
			...
			if ( $pf_found_posts > 0 && $pf_found_posts > $pf_per_page + $pf_offset ) {
				?>
                <a href="#" class="button"><?php esc_html_e( 'Load More', 'lebe' ); ?></a>
				<?php
			} else {
				?>
                <span class="button disabled"><?php esc_html_e( 'No More Products!', 'lebe' ); ?></span>
				<?php
			}
			?>
        </nav>
		...
		echo '<div class="lebe-ajax-load" data-mode="grid" data-2nd_page_url="' . esc_url( get_next_posts_page_link( $total ) ) . '" data-cur_page="1" data-total_page="' . esc_attr( $total ) . '" data-load-more=\'{"page":"' . esc_attr( $total ) . '","container":"product-grid","layout":"' . esc_attr( $enable_loadmore ) . '"}\'>';
		next_posts_link( esc_html__( 'Load More', 'lebe' ), $total );
		echo '</div>';
		echo '</div>';
	}
} else {
