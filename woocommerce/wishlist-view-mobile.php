            ...
            <div class="wishlist-title <?php if ( $is_custom_list ) {
                ...
                <?php if ( $is_custom_list ): ?>
                    <a class="btn button show-title-form">
                        ...
                        <?php esc_html_e( 'Edit title', 'lebe' ) ?>
                    </a>
                <?php endif; ?>
            </div>
            <?php if ( $is_custom_list ): ?>
        ...
        <table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination ); ?>"
            ...
            <tr>
                ...
                <th class="product-thumbnail">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product', 'lebe' ) ) ?>
                </th>
                ...
                <?php if ( $show_price ) : ?>
                    ...
                    <th class="product-price">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Price', 'lebe' ) ) ?>
                    </th>
            ...
            if ( count( $wishlist_items ) > 0 ) :
                        ...
                        <tr id="yith-wcwl-row-<?php echo esc_attr( $item['prod_id'] ); ?>"
                            ...
                            <td class="product-thumbnail">
                                <a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">
                                    <?php echo lebe_html_output( $product->get_image() ); ?>
                                </a>
                            </td>
                            ...
                            <?php if ( $show_price ) : ?>
                                <td class="product-price">
                                    ...
                                    if ( $product->get_price() ) {
                                        echo lebe_html_output( $product->get_price_html() );
                                    } else {
                                        echo apply_filters( 'yith_free_text', esc_html__( 'Free!', 'lebe' ) );
                                    }
                            ...
                            <?php if ( $show_stock_status ) : ?>
                                <td class="product-stock-status">
                                    ...
                                    if ( $stock_status == 'out-of-stock' ) {
                                        echo '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'lebe' ) . '</span>';
                                    } else {
                                        echo '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'lebe' ) . '</span>';
                                    }
                                    ?>
                                </td>
                            <?php endif ?>
                            ...
                            <?php if ( $show_last_column ): ?>
                                <td class="product-add-to-cart">
                                    ...
                                    if ( $show_dateadded && isset( $item['dateadded'] ) ):
                                        echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'lebe' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
                                    endif;
                                    ?>
                                    ...
                                    <?php if ( $available_multi_wishlist && is_user_logged_in() && count( $users_wishlists ) > 1 && $move_to_another_wishlist && $is_user_owner ): ?>
                                        <select class="change-wishlist selectBox">
                                            <option value=""><?php esc_html_e( 'Move', 'lebe' ) ?></option>
                                            <?php
                                            foreach ( $users_wishlists as $wl ):
                                                ...
                                                <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
                                                    ...
                                                    if ( $wl['wishlist_privacy'] == 1 ) {
                                                        $wl_privacy = esc_html__( 'Shared', 'lebe' );
                                                    } elseif ( $wl['wishlist_privacy'] == 2 ) {
                                                        $wl_privacy = esc_html__( 'Private', 'lebe' );
                                                    } else {
                                                        $wl_privacy = esc_html__( 'Public', 'lebe' );
                                                    }
                                    ...
                                    <?php if ( $is_user_owner && $repeat_remove_button ): ?>
                                        <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>"
                                           ...
                                           title="<?php esc_attr_e( 'Remove this product', 'lebe' ); ?>"><?php esc_html_e( 'Remove', 'lebe' ); ?></a>
                                    <?php endif; ?>
                                </td>
                            <?php endif; ?>
                            <?php if ( $is_user_owner ): ?>
                                <td class="product-remove">
                                    <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ); ?>"
                                       ...
                                       title="<?php esc_attr_e( 'Remove this product', 'lebe' ); ?>">&times;
                                        <span><?php esc_html_e( 'remove', 'lebe' ); ?></span></a>
                                </td>
                            <?php endif; ?>
                        </tr>
                        <?php
                ...
                <tr>
                    <td colspan="<?php echo esc_attr( $column_count ); ?>"
                        class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products were added to the wishlist', 'lebe' ) ); ?></td>
                </tr>
                <?php
            endif;
            ...
            if ( ! empty( $page_links ) ) : ?>
                <tr class="pagination-row">
                    <td colspan="<?php echo esc_attr( $column_count ); ?>"><?php echo lebe_html_output( $page_links ); ?></td>
                </tr>
            <?php endif ?>
            </tbody>
            <?php if ( $show_cb ) : ?>
                    ...
                    <?php if ( $show_cb ) : ?>
                        <div class="custom-add-to-cart-button-cotaniner">
                            <a href="<?php echo esc_url( add_query_arg( array(
                               ...
                               id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', esc_html__( 'Add the selected products to the cart', 'lebe' ) ) ?></a>
                        </div>
                    <?php endif; ?>
                        ...
                        <div class="ask-an-estimate-button-container">
                            <a href="<?php echo ( is_user_logged_in() || $additional_info ) ? '#ask_an_estimate_popup' : $ask_estimate_url ?>"
                               class="btn button ask-an-estimate-button" <?php echo ( true == $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> >
                                ...
                                <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', esc_html__( 'Ask for an estimate', 'lebe' ) ) ?>
                            </a>
                        </div>
                    <?php endif; ?>
    ...
    <div id="ask_an_estimate_popup">
        <form action="<?php echo esc_url( $ask_estimate_url ); ?>" method="post"
            ...
            <?php if ( ! is_user_logged_in() ): ?>
                <label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', esc_html__( 'Your email', 'lebe' ) ) ?></label>
